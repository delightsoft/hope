...
(function () {
  ...
  link = function link(config, noHelpers) {
    var EMPTY_FLAT_MAP, EMPTY_LIST, EMPTY_MAP, EMPTY_MAP_WITH_TAGS, EMPTY_MASK, EMPTY_TAGS, api, _deepFreeze, doc, docPrefix, freeze, j, l, len, len1, len2, len3, len4, linkFields, linkFieldsWithHelpers, linkFlatMap, linkSortedMap, linkTags, m, method, n, o, ref, ref1, ref2, ref3, ref4, state, transition;
    ...
    linkFieldsWithHelpers = function linkFieldsWithHelpers(obj, prop, prefix) {
      ...
      if (!noHelpers) {
        _assignKey = function assignKey(fields, prefix) {
          ...
          for (j = 0, len = ref.length; j < len; j++) {
            field = ref[j];
            nextLevelPrefix = "".concat(prefix, ".field.").concat(field.name);
            field.$$key = field.hasOwnProperty('fields') ? "".concat(nextLevelPrefix, ".label") : nextLevelPrefix;
    ...
    for (j = 0, len = ref.length; j < len; j++) {
      ...
      if (!noHelpers) {
        docPrefix = doc.name;
        doc.$$key = "".concat(docPrefix, ".title");
      }
      ...
      if (!noHelpers) {
        doc.actions.$$list.forEach(function (a) {
          a.$$key = "".concat(docPrefix, ".action.").concat(a.name);
        });
        doc.states.$$list.forEach(function (s) {
          s.$$key = "".concat(docPrefix, ".state.").concat(s.name);
        });
      }
