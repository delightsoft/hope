...
(function () {
  ...
  link = function link(config, noHelpers, opts) {
    var EMPTY_FLAT_MAP, EMPTY_LIST, EMPTY_MAP, EMPTY_MAP_WITH_TAGS, EMPTY_MASK, EMPTY_TAGS, api, apiKey, _deepFreeze, doc, docKey, freeze, j, l, len, len1, len2, len3, len4, linkFields, linkFieldsWithHelpers, linkFlatMap, linkSortedMap, linkTags, m, method, methods, n, o, ref, ref1, ref2, ref3, ref4, state, transition;
    ...
    for (j = 0, len = ref.length; j < len; j++) {
      ...
      if (!noHelpers) {
        ...
        doc.$$access = $$accessBuilder(doc, 'fields', methods && methods.docs[doc.name] && methods.docs[doc.name].access, true);
        doc.$$validate = $$validateBuilder(doc, 'fields', doc.$$access, methods && methods.docs[doc.name] && methods.docs[doc.name].validate);
        doc.$$editValidateBuilder = $$editValidateBuilderBuilder(doc, 'fields', doc.$$access, methods && methods.docs[doc.name] && methods.docs[doc.name].validate);
    ...
    for (n = 0, len3 = ref3.length; n < len3; n++) {
      ...
      for (o = 0, len4 = ref4.length; o < len4; o++) {
        ...
        if (!noHelpers) {
          method.arguments.$$access = $$accessBuilder(method, 'arguments', methods && methods.api[api.name] && methods.api[api.name][method.name] && methods.api[api.name][method.name].argAccess);
          method.arguments.$$validate = $$validateBuilder(method, 'arguments', method.arguments.$$access, methods && methods.api[api.name] && methods.api[api.name][method.name] && methods.api[api.name][method.name].argValidE);
          method.arguments.$$editValidateBuilder = $$editValidateBuilderBuilder(method, 'arguments', method.arguments.$$access, method.arguments.$$access, methods && methods.api[api.name] && methods.api[api.name][method.name] && methods.api[api.name][method.name].argValidE);
          method.result.$$access = $$accessBuilder(method, 'result', methods && methods.api[api.name] && methods.api[api.name][method.name] && methods.api[api.name][method.name].resultAccess);
          method.result.$$validate = $$validateBuilder(method, 'result', method.result.$$access, methods && methods.api[api.name] && methods.api[api.name][method.name] && methods.api[api.name][method.name].resultValidate);
          method.result.$$editValidateBuilder = $$editValidateBuilderBuilder(method, 'result', method.arguments.$$access, method.arguments.$$access, methods && methods.api[api.name] && methods.api[api.name][method.name] && methods.api[api.name][method.name].resultValidate);
        }
