...
(function () {
  ...
  link = function link(config, noHelpers) {
    var EMPTY_FLAT_MAP, EMPTY_LIST, EMPTY_MAP, EMPTY_MAP_WITH_TAGS, EMPTY_MASK, EMPTY_TAGS, api, _deepFreeze, doc, freeze, j, l, len, len1, len2, len3, len4, linkFields, linkFieldsWithHelpers, linkFlatMap, linkSortedMap, linkTags, m, method, n, o, ref, ref1, ref2, ref3, ref4, state, transition;
      ...
      if (!noHelpers) {
        _assignKey = function assignKey(fields, prefix) {
          ...
          for (j = 0, len = ref.length; j < len; j++) {
            field = ref[j];
            field.$$key = nextLevelPrefix = "".concat(prefix, ".field.").concat(field.name);
    ...
    for (j = 0, len = ref.length; j < len; j++) {
      doc = ref[j];
      linkFieldsWithHelpers(doc, 'fields', "doc.".concat(doc.name));
      ...
      if (!noHelpers) {
        doc.actions.$$list.forEach(function (a) {
          a.$$key = "doc.".concat(doc.name, ".action.").concat(a.name);
        });
        doc.states.$$list.forEach(function (s) {
          s.$$key = "doc.".concat(doc.name, ".state.").concat(s.name);
        });
      }
