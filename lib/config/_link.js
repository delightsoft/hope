...
(function () {
  ...
  linkSortedMap = function linkSortedMap(collection, noIndex, noFreeze, noHelpers) {
  ...
  linkFlatMap = function linkFlatMap(collection, prop, noIndex, noMask, noHelpers) {
    ...
    _linkLevel2 = function _linkLevel(level) {
      ...
      return linkSortedMap({
        list: level
      }, true, prefix === '', noHelpers); // _linkLevel
    };
  ...
  linkTags = function linkTags(res, collection, noHelpers) {
  ...
  linkUDTypes = function linkUDTypes(config, list, noHelpers) {
    ...
    for (j = 0, len = list.length; j < len; j++) {
      ...
      if (type.hasOwnProperty('enum')) {
        // linkUDTypes =
        type["enum"] = linkSortedMap(type["enum"], true, false, noHelpers);
      }
    }
  };

  linkFields = function linkFields(config, list, noHelpers) {
    ...
    for (j = 0, len = list.length; j < len; j++) {
      ...
      if (field.hasOwnProperty('enum')) {
        field["enum"] = linkSortedMap(field["enum"], true, noHelpers);
      }
