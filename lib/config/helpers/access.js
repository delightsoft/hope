...
(function () {
  ...
  $$accessBuilder = function $$accessBuilder(type, fieldsProp, access, addActions) {
    ...
    res = typeof access === 'function' ? function (fields) {
      ...
      if (addActions) {
        r.view = freezeBitArray(r.view ? r.view.add(type[fieldsProp].$$calc('#system-options', {
          strict: false
        })) : type[fieldsProp].$$tags.all);
        r.update = freezeBitArray(r.update ? r.update.add(type[fieldsProp].$$calc('id,rev,state,deleted', {
          strict: false
        })) : type[fieldsProp].$$tags.all);
