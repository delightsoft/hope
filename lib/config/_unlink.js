...
(function () {
  var lightClone, unlink, unlinkAction, unlinkField, unlinkFlatMap, unlinkSortedMap, unlinkTags;
  ...
  unlinkTags = function unlinkTags(tags) {
    ...
    for (k in tags) {
      ...
      if (!~['all', 'none', 'required'].indexOf(k)) {
        res[k] = v._mask;
      }
    }
  ...
  unlinkField = function unlinkField(field) {
    ...
    if (field.regexp) {
      field.regexp = field.regexp.toString();
    }

    delete field._validate;
  };

  unlinkAction = function unlinkAction(action) {
    if (action.hasOwnProperty('arguments')) {
      action.arguments = unlinkFlatMap(action.arguments, 'fields', unlinkField);
    }

    if (action.hasOwnProperty('result')) {
      action.result = unlinkFlatMap(action.result, 'fields', unlinkField);
    }
  };

  unlink = function unlink(config) {
    var api, i, j, l, len, len1, len2, len3, len4, m, method, n, newConfig, newDoc, newState, newTran, ref1, ref2, ref3, ref4, ref5, ref6;
    ...
    if (!(((ref4 = newConfig.api) != null ? ref4.$$list.length : void 0) > 0)) {
      delete newConfig.api;
    } else {
      newConfig.api = unlinkSortedMap(newConfig.api);
      ref5 = newConfig.api.list;
