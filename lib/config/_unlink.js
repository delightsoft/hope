...
(function () {
    ...
    _unlinkLevel = function unlinkLevel(level) {
      ...
      for (i = 0, len = level.length; i < len; i++) {
        ...
        if (clone.hasOwnProperty(prop)) {
          clone[prop] = _unlinkLevel(clone[prop]);
        }
  ...
  unlinkUDType = function unlinkUDType(field) {
    var ref;
  ...
  unlinkMethods = function unlinkMethods(methods) {
    ...
    for (i = 0, len = ref1.length; i < len; i++) {
      ...
      if (res.input.$$list.length > 0) {
        res.input = unlinkMap(method.input);
      }

      if (res.output.$$list.length > 0) {
        res.output = unlinkMap(method.output);
      }
  ...
  unlink = function unlink(config) {
    ...
    if (!(config.udtypes.$$list.length > 0)) {
      ...
      for (m = 0, len3 = ref4.length; m < len3; m++) {
        ...
        if (!(api.methods.$$list.length > 0)) {
          ...
          for (n = 0, len4 = ref5.length; n < len4; n++) {
            ...
            if (method.input.$$list.length > 0) {
              method.input = unlinkFlatMap(method.input, 'fields', unlinkField);
            }

            if (method.output.$$list.length > 0) {
              method.output = unlinkFlatMap(method.output, 'fields', unlinkField);
            }
          }
        }
      }
