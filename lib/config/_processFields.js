...
(function () {
  var Result, bitArray, compileTags, compileType, copyExtra, deepClone, extraProps, flatMap, processCustomValidate, processFields, typeProps;
  ...
  processFields = function processFields(result, doc, config) {
    ...
    var udtContext = arguments.length > 5 ? arguments[5] : undefined;
    ...
    return result.context(Result.prop(fieldsProp), function () {
      // processFields
      var _processLevel2, flatMapOpts, resValue;
      ...
      if (!noSystemItems) {
        flatMapOpts.before = [{
          name: 'id',
          type: 'nanoid',
          tags: 'field, system, index, unique, hide'
        }, {
          name: 'rev',
          type: 'int',
          init: 0,
          tags: 'field, system, hide'
        }];
        flatMapOpts.after = [{
          name: 'options',
          type: 'json',
          tags: 'field, system, index, hide'
        }, {
          name: 'created',
          type: 'timestamp',
          tags: 'field, system, index, hide'
      ...
      if (!result.isError) {
        _processLevel2 = function _processLevel(level) {
          ...
          result.context(function (path) {
            ...
            var i, j, k, len, len1, len2, parentUdt, prop, ref, ref1, skipProcessSublevel, src, udt, udtList;
            ref = level.$$list;
            ...
            for (i = 0, len = ref.length; i < len; i++) {
              ...
              if (field.hasOwnProperty('udType') && _typeof(config.udtypes) === 'object') {
                if (!config.udtypes.hasOwnProperty(field.udType)) {
                  ...
                  compileType(result, src, field, {
                    context: 'field',
                    udType: true // перекомпилируем, на случай если переопределены свойства базового типа

                  });

                  if (src.fields) {
                    skipProcessSublevel = true;
                    field.fields = deepClone(src.fields, {
                      all: true
                    });
                  }
              ...
              if (processCustomValidate(result, field.$$src, level, void 0, (ref1 = config.$$src) != null ? ref1.validators : void 0)) {
                // result.context
                // _processLevel =
        ...
        if (!result.isError) {
          if (udtContext) {} else {
            ...
            flatMap.index(result, resValue, 'fields', {
              mask: true
            });
            ...
            if (!result.isError) {
              flatMap.finish(result, resValue, 'fields', {
                skipProps: ['type', 'tags', 'required', 'null']
              });
            }
          }
        }
