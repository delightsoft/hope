...
(function () {
  ...
  calcTags = _require.calc;

  processStates = function processStates(result, doc, fields, actions) {
    var res;
    ...
    result.context(Result.prop('states'), function () {
      ...
      if (!result.isError) {
        ...
        result.context(function (path) {
          return Result.prop(state.name)(path);
        }, function () {
          var i, len, ref;
          ref = res.$$list;
          ...
          for (i = 0, len = ref.length; i < len; i++) {
            ...
            result.context(Result.prop('view'), function () {
              if (!state.$$src.hasOwnProperty('view')) {
                ...
                state.view = calcTags(result, fields, state.$$src.view); // result.context
              }
            });
            result.context(Result.prop('update'), function () {
              if (!state.$$src.hasOwnProperty('update')) {
                ...
                state.update = calcTags(result, fields, state.$$src.update); // result.context
              }
            });
            result.context(Result.prop('transitions'), function () {
              ...
              if (!state.$$src.hasOwnProperty('transitions')) {
                ...
                if (!result.isError) {
                  transition = void 0;
                  result.context(function (path) {
                    return Result.prop(transition.name)(path);
                  }, function () {
                    var j, len1, ref1;
