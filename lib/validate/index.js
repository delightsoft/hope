...
(function () {
  var Result, _moment, _addValidate, cvResult, momentLdr, processCustomValidate, validate, validateStructureBuilder;
          ...
          if (!fieldName.startsWith('$$')) {
            if (!type[fieldsProp].hasOwnProperty(fieldName)) {
              ...
              if (!viewMask.get(field.$$index)) {
                ...
                results.push(err = field._validate(result, fieldValue, value, viewMask, requiredMask, _typeof(field) === 'object' && field !== null && !Array.isArray(field) ? field.$$touched : void 0, strict) || err);
              } else {
                results.push(void 0);
              }
            }
  ...
  validate = function validate(fieldDesc, fields, validators) {
    ...
    f = function () {
      switch (fieldDesc.type) {
        ...
        case 'structure':
          return validateStructureBuilder(fieldDesc, 'fields', validators);

        case 'subtable':
          return function () {
            var validateStructure;
            validateStructure = validateStructureBuilder(fieldDesc, 'fields', validators);
            return function (result, value, fieldsLevel, viewMask, requiredMask, strict) {
              var err, i;
              ...
              result.context(function (path) {
                ...
                for (i = j = 0, len1 = value.length; j < len1; i = ++j) {
                  row = value[i];
                  results.push(err = validateStructure(result, row, void 0, viewMask, requiredMask, row.$$touched, strict) || err);
                }
