...
(function () {
  ...
  builtInTypes = ['nanoid', 'string', 'text', 'boolean', 'integer', 'double', 'decimal', 'time', 'date', 'timestamp', 'json', 'blob', 'uuid', 'enum', 'structure', 'subtable', 'refers', 'dsvalue'];
    ...
    if (!(builtInTypes.indexOf(type) >= 0)) {
      if (reservedTypes.indexOf(type) >= 0) {
        ...
        result.context(function (path) {
          ...
          var i, len, ref, results; // udType can have null and required in a field definition
          ...
          for (i = 0, len = typeProps.length; i < len; i++) {
            ...
            if (fieldDesc.hasOwnProperty(prop) && ((ref = !prop) === 'null' || ref === 'required')) {
