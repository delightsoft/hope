...
(function () {
  var Result, builtInTypes, checkEnumName, checkUDTypeName, compile, copyExtra, extraProps, invalidArg, isResult, processExraProps, reservedTypes, setRequiredProp, sortedMap, takeBoolean, takeEnum, takeNoneNegativeInt, takePositiveInt, takeString, takeStringOrArrayOfStrings, tooManyArgs, typeProps;
  ...
  builtInTypes = ['nanoid', 'string', 'text', 'boolean', 'integer', 'double', 'decimal', 'time', 'date', 'timestamp', 'json', 'blob', 'uuid', 'enum', 'structure', 'subtable', 'refers'];
  reservedTypes = ['long', 'float', 'timetz', 'timestamptz'];

  compile = function compile(result, fieldDesc, res, opts) {
    var enumProp, fieldsProp, i, j, len, len1, lengthProp, nullProp, options, optionsEnd, optionsIndex, optionsLen, optsContext, optsUdtype, precisionProp, prop, refersProp, requiredProp, scaleProp, sourceType, type, valueClass;
    ...
    if (!(opts != null ? opts.hasOwnProperty('context') : void 0)) {
      ...
      if (!((optsContext = opts.context) === 'field' || optsContext === 'udtype')) {
        invalidArg('opts.context', opts.context);
      }
    }

    optsUdtype = opts != null ? opts.udType : void 0;

    if (!!fieldDesc.hasOwnProperty('$$tags')) {
      throw new Error("fieldDesc was already process by tags/compile()");
    }

    result.isError = false; // Определяем тип.  То что в скобках выделяем в options.
    ...
    result.context(function (path) {
      ...
      for (i = 0, len = typeProps.length; i < len; i++) {
        ...
        switch (prop) {
          ...
          case 'scale':
            if (ok = type === 'decimal') {
              scaleProp = takeNoneNegativeInt(result, fieldDesc.scale);
            }
    ...
    switch (type) {
      ...
      case 'decimal':
        res.precision = typeof precisionProp === 'number' ? !(1 <= precisionProp && precisionProp <= 15) ? result.error(Result.prop('precision'), 'dsc.precisionOutOfRange', {
          value: precisionProp,
          min: 1,
          max: 15
        }) : res.precision = precisionProp : 15;

        if (!result.isError) {
          res.scale = typeof scaleProp === 'number' ? !(0 <= scaleProp && scaleProp <= res.precision) ? result.error(Result.prop('scale'), 'dsc.scaleOutOfRange', {
            value: scaleProp,
            min: 0,
            max: res.precision
          }) : res.scale = scaleProp : 0;
        }
      ...
      case 'structure':
        ...
        if (!(optsUdtype || res.hasOwnProperty('fields'))) {
  ...
  takeNoneNegativeInt = function takeNoneNegativeInt(result, value) {
    if (!(typeof value === 'number' && !isNaN(value) && Number.isInteger(value) && value >= 0)) {
      result.error('dsc.invalidValue', {
        value: value
      });
  ...
  takeEnum = function takeEnum(result, value) {
    ...
    if (_typeof(value) === 'object' && value !== null && value.hasOwnProperty('$$list')) {
      // это случается при использовании enum в udTypes
      return value;
    }
