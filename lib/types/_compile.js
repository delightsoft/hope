...
(function () {
  var Result, builtInTypes, checkItemName, compile, invalidArg, invalidArgValue, isResult, reservedTypes, setRequiredProp, sortedMap, takeBoolean, takeEnum, takeFields, takePositiveInt, takeString, takeStringOrArrayOfStrings, tooManyArgs, typeProps;
  ...
  sortedMap = require('../sortedMap'); //typeProps = ['length', 'enum', 'precision', 'scale', 'fields', 'refers', 'valueClass', 'null']

  typeProps = ['length', 'enum', 'fields', 'refers', 'valueClass', 'null'];
  builtInTypes = ['string', 'text', 'boolean', // 'integer', 'long', 'float', 'double', 'decimal'
  'integer', 'double', 'decimal', 'time', 'date', 'dateonly', 'now', 'json', 'blob', 'uuid', 'enum', 'structure', 'subtable', 'refers', 'dsvalue'];
  reservedTypes = ['long', 'float'];

  compile = function compile(result, fieldDesc, res, opts) {
    var enumProp, fieldsProp, lengthProp, nullProp, options, optionsEnd, optionsIndex, optionsLen, optsContext, precisionProp, prop, refersProp, scaleProp, sourceType, type, valueClass;
    ...
    if (!(builtInTypes.indexOf(type) >= 0)) {
      if (reservedTypes.indexOf(type) >= 0) {
        ...
        result.context(function (path) {
          ...
          var i, len, prop, results; // udType can have null in a field definition
    ...
    prop = lengthProp = enumProp = precisionProp = scaleProp = fieldsProp = nullProp = refersProp = valueClass = void 0;
  ...
  takeFields = function takeFields(result, value) {
    return sortedMap(result, value);
